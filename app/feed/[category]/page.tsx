...
const siddurData: Record<string, { title: string, subtitle?: string, verses: { id: string, hebrew: string, english: string, isSection?: boolean, isImage?: boolean, imageUrl?: string }[] }> = {
  ...
  mincha: {
    ...
    verses: [
      { id: "m1", hebrew: "אַשְׁרֵי יוֹשְׁבֵי בֵיתֶךָ, עוֹד יְהַלְלוּךָ סֶּלָה.", english: "Fortunate are those who dwell in Your house; they will forever praise You." }
    ]
  },
  maariv: {
    title: "Evening Prayers",
    verses: [
      { id: "ev1", hebrew: "וְהוּא רַחוּם יְכַפֵּר עָוֹן וְלֹא יַשְׁחִית, וְהִרְבָּה לְהָשִׁיב אַפּוֹ וְלֹא יָעִיר כָּל חֲמָתוֹ.", english: "But He, being merciful, forgives iniquity and does not destroy; He frequently turns away His anger and does not arouse all His wrath." }
    ]
  }
};
...
export default function SiddurView() {
  ...
  const tefillah = typeof params?.tefillah === 'string' ? params.tefillah : "shacharit";
  const selectedTefillah = siddurData[tefillah] || siddurData["shacharit"];
  ...
  const toggleAudio = () => {
    if (audioRef.current) {
      if (isPlaying) {
        ...
        audioRef.current.play().catch(() => console.warn('Ambient audio stream missing or restricted.'));
      }
      ...
      <nav className={`zen-nav ${scrolled ? 'scrolled' : ''}`}>
        <div className="nav-content">
          ...
          <div className="spacer">
            <button onClick={toggleAudio} className="audio-toggle" aria-label="Toggle ambient sound">
