  ...
  return (
    <div className="min-h-[100dvh] bg-black text-white font-sans selection:bg-white/20">
      ...
      <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-black/50 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 md:gap-6">
            <Link
              href="/chat"
              className="text-[10px] md:text-xs text-zinc-400 hover:text-white uppercase tracking-widest font-medium transition-colors"
            >
              Chat
            </Link>
            <Link
      ...
      <main className="relative z-10">
        ...
        <section className="min-h-[100dvh] pt-28 pb-12 flex max-w-7xl mx-auto px-6 lg:max-h-[1100px]">
          <div className="w-full my-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">

            {/* Left Pane: Copy & CTA */}
            <div className="space-y-8 max-w-xl order-2 lg:order-1 flex flex-col items-center lg:items-start text-center lg:text-left">
              ...
              <div className="pt-4 flex flex-row items-center gap-4 sm:gap-6 flex-wrap">
                <button
                  onClick={() => window.scrollTo({ top: window.innerHeight, behavior: 'smooth' })}
                  className="px-5 sm:px-6 py-2.5 sm:py-3 rounded-full bg-white text-black text-xs tracking-wider uppercase font-bold hover:bg-zinc-200 transition-colors shadow-[0_0_20px_rgba(255,255,255,0.15)] whitespace-nowrap"
                >
                  Explore Integration
                </button>
                <Link
                  href="/chat"
                  className="text-zinc-400 hover:text-white transition-colors text-xs sm:text-sm font-medium group flex items-center gap-1.5 whitespace-nowrap"
                >
                  Open Full Screen Chat <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </Link>
              </div>
            </div>
            ...
            <div className="relative w-full flex justify-center lg:justify-end order-1 lg:order-2">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/10 via-transparent to-transparent blur-3xl opacity-50 rounded-full hidden lg:block" />
